...
**已完成：阶段 0–7、阶段 8A/8B 服务端 + 管理端；阶段 5–8B C 端接口已完成。小程序待接入阶段 3–8。**
...
| 8 分销 + 统计 | **8A/8B 已完成** | 身份/等级/代理、二级关系与佣金账本已落地；8C–8E 待开发 |
| 9 联调/真实支付/上线 | 未开始 | |
...
- 核销后佣金转可用、直属有效销售额累计；达到 10 万自动升为荣誉股东，只升不自动降
- 已完成订单支持整单退款；退款冲正佣金和直属业绩，同时撤销核销产生的会员积分、累计消费和等级收益
- C 端提供申请、资料、绑定推荐人、佣金明细和团队接口；本阶段不开发小程序页面
- 数据与菜单：`sql/mysql/biz_phase8_distribution.sql`、`sql/mysql/menu_phase8_distribution.sql`
- 验证：真实 122,152 元订单生成两级佣金，核销升级、退款冲正和不降级闭环通过

### 待开发：8C–8E

- 8C：结算周期、待结算单、提现申请/自动打款、微信打款和限额
...
1. 读本文「当前进度」：阶段 8A/8B 服务端、管理端和 C 端接口已完成，小程序待补。
2. 按当前安排进入阶段 8C 结算与提现；小程序阶段 3–8 暂缓。
