...
**已完成：阶段 0、1、2。下一步：阶段 3（购物车 + 订单 + 核销）。**
...
| 3 购物车/订单/核销 | **下一步** | 计价管道、领域事件、Mock 支付、超时取消 |
| 4 支付抽象 + 个人中心 | 未开始 | P0 闭环验收 |
| 5 等级/积分/优惠券 | 未开始 | |
| 6 医生预约 | 未开始 | |
| 7 首页装修 + 拼团 | 未开始 | |
| 8 分销 + 统计 | 未开始 | |
...
flowchart LR
    ...
    subgraph server [服务端 youlai-nest]
        ...
        BizLayer[业务层 member/product 已建；cart/order 待建]
    end
...
### 阶段 4：支付抽象 + 个人中心（P0 收尾）

- `src/payment`：把 Mock 收敛为统一驱动（下单/回调/查询/退款），预留 wechatpay
- 小程序：支付结果页、个人中心（头像昵称、订单入口）
- 管理端：会员列表/360 视图（阶段 1 已有查询接口）
...
1. 读本文「当前进度」，确认仍停在阶段 3。
2. 读 `sql/mysql/biz_p0.sql` 里 `cart` / `biz_order` / `biz_order_item`。
3. 对照现有 `src/product`、`src/member` 的模块分层写 `src/cart`、`src/order`。
4. 阶段内仍遵守：服务端 → 管理端 → 小程序。
5. 每阶段验收通过再提交；不要把 Docker 数据目录、`.env` 密钥提交进 Git。
