...
- 渠道创建发生在支付意图提交之后，重复请求以同一 `paymentNo` 重新取得调起参数。
- 重复支付成功确认不重复推进订单、增加销量或使用权益。
- 待确认支付和处理中的退款每分钟按渠道幂等键补偿同步；多实例任务由 Redis 锁串行执行。
- 生产环境禁止模拟支付确认。
...
- 退款意图先提交数据库，再在事务外调用渠道；普通重试和补偿任务始终复用同一 `refundNo`。
- 渠道返回处理中或失败时保留退款单状态，只有渠道确认成功后才在独立事务推进订单和权益回退。
- 超时取消后才到达的支付成功结果自动创建原路退款；退款完成只改变订单终态，不重复回补库存和权益。
...
| 渠道退款处理中或失败                 | 保留退款状态，不推进订单成功状态并进入补偿/异常待办 |
...
- **Given** 渠道退款处理中或调用后进程退出，**When** 补偿任务重试，**Then** 复用原退款号且不重复出款。
- **Given** 订单已超时取消后渠道才确认成功，**When** 同步支付结果，**Then** 自动原路退款且库存和权益只回补一次。
