...
> 模块状态：阶段 8A–8E 服务端与 B 端已实现；小程序我的分销已接入，真实账号联调待验收
...
| 角色         | 使用场景                                         | 当前支持                       |
| ------------ | ------------------------------------------------ | ------------------------------ |
| 普通会员     | 使用邀请码绑定推荐人                             | 无分销入口；推荐绑定接口可用   |
| 代理商       | 查看资料、佣金、团队、账户、提现、任务和直属业绩 | 页面已接入，真实账号联调待验收 |
...
- 后台创建代理并管理身份；历史申请的后台审核能力保留，C 端不再接受申请。
...
1. 后台选择会员并配置真实姓名、联系信息、代理类型、等级和可选上级代理。
2. 创建成功后获得已通过代理身份和唯一邀请码。
3. 小程序个人中心读取本人代理资料后展示「我的分销」。
4. 普通会员可使用邀请码首次绑定直属推荐代理；绑定关系不会开通代理身份。
5. 关系产生佣金后冻结，后续交易继续沿用既有归属。
6. 后台停用代理后，小程序关闭团队和推广入口，保留历史账目与按规则提现。
...
| 状态   | 含义                     | 产品行为                               |
| ------ | ------------------------ | -------------------------------------- |
| 待审核 | 历史代理申请             | 不展示 C 端分销入口                    |
| 已通过 | 审核通过并可正常开展分销 | 可产生佣金、查看任务和申请提现         |
| 已驳回 | 历史申请未通过           | 不展示 C 端入口，身份仅由后台处理      |
...
- 无有效代理身份的会员查看代理专属数据时返回空结果或提示代理不存在，不暴露他人信息。
...
| 场景                              | 处理                                 |
| --------------------------------- | ------------------------------------ |
| 调用 C 端代理申请或重复绑定邀请码 | 申请接口不存在；推荐关系拒绝重复绑定 |
...
| 能力                   | 服务端 | B 端   | C 端                         |
| ---------------------- | ------ | ------ | ---------------------------- |
| 类型、等级、代理、团队 | 已实现 | 已接入 | 已接入，待真实账号验收       |
| 推荐关系与佣金账本     | 已实现 | 已接入 | 佣金已接入；推荐绑定页待接入 |
| 周期结算与提现         | 已实现 | 已接入 | 已接入，待真实账号验收       |
| 销售任务               | 已实现 | 已接入 | 已接入，待真实账号验收       |
| 销售统计与导出         | 已实现 | 已接入 | 已接入，待真实账号验收       |

小程序已接入分销首页、佣金、团队、钱包提现、业绩、任务与只读代理资料。素材中心、收款资料编辑、自动微信转账和任务奖励不属于已实现范围。真实账号联调与微信真机验收仍待完成。

## 11. 验收标准

- **Given** 会员尚无代理身份，**When** 后台创建代理并指定有效类型和等级，**Then** 会员获得已通过代理身份和唯一邀请码，C 端展示分销入口。
